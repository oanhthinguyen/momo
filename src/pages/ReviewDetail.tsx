...
export default function ReviewDetail() {
  ...
  if (localReview) {
    ...
    if (typeof localReview.ingredients === 'string') {
      data.ingredients = localReview.ingredients.split(',').map((s: string) => ({ name: s.trim(), amount: 'Có' })).filter((i: any) => i.name);
    } else {
      data.ingredients = localReview.ingredients;
    }
  } else {
    // 2. Check mock data
    ...
    if (mockData) {
      ...
      if (typeof data.ingredients === 'string') {
        data.ingredients = data.ingredients.split(',').map((s: string) => ({ name: s.trim(), amount: 'Có' })).filter((i: any) => i.name);
      }
    }
  }
          ...
          <div className="ingredients-section animate-fade-in" style={{ background: '#f8fafc', padding: '24px', borderRadius: '16px', marginTop: '32px', marginBottom: '32px', border: '2px solid #e2e8f0' }}>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '12px', alignItems: 'center' }}>
              {data.ingredients.map((ing: any, i: number) => (
                <li key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '1.1rem', fontWeight: 700, width: '100%', maxWidth: '300px' }}>
                  <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: i % 2 === 0 ? '#1d4ed8' : '#e11d48', flexShrink: 0 }}></div>
                  <span style={{ color: i % 2 === 0 ? '#1d4ed8' : '#e11d48', minWidth: '120px' }}>{ing.name}:</span>
                  <span style={{ color: i % 2 === 0 ? '#1d4ed8' : '#e11d48' }}>{ing.amount}</span>
                </li>
              ))}
            </ul>
          </div>
